...
def resample_polyline(xy, step_px=2.0):
    # xy: (N,2) 
...
if __name__ == "__main__":
    ...
    for idx in range(len(lane_ratios)):
        ...
        # ********************************************************
        tol = max(0.15, lane_ratios[idx]*0.25)
        valid_ratio = (np.abs(valid_pts[:, -1] - lane_ratios[idx]) < tol)
        ...
        # Resample polyline *******************************
        lane = resample_polyline(lane.astype(float), step_px=2.0).astype(int)
        # *************************************************
        if clockwise:
            lane = np.flipud(lane)
