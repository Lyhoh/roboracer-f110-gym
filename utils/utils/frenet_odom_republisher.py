...
class FrenetOdomRepublisher(Node):
    ...
    def global_waypoints_cb(self, msg: WaypointArray) -> None:
        ...
        # 如果你的 Wpnt 里有 psi_rad，可以一并传给 FrenetConverter：
        # has_psi = hasattr(msg.wpnts[0], "psi_rad")
        # if has_psi:
        #     psis = [wp.psi_rad for wp in msg.wpnts]
        #     self.converter = FrenetConverter(np.array(xs), np.array(ys), np.array(psis))
        # else:
        #     self.converter = FrenetConverter(np.array(xs), np.array(ys))

        # 暂时不传 psi（和你当前 csv 示例一致）
    ...
    def odom_cb(self, msg: Odometry) -> None:
        ...
        if self.converter.waypoints_psi is not None:
            ...
            # 没有 psi 信息时，简单地把 vx 看成沿轨迹方向的速度
            v_s = float(vx)
            v_d = 0.0

        # --- Build Frenet odom message ---
        frenet_msg = Odometry()
        frenet_msg.header = msg.header  # 继承时间戳和 frame_id

        # (s, d) 存在 pose.position.x / y
        frenet_msg.pose.pose.position.x = s
        frenet_msg.pose.pose.position.y = d

        # v_s, v_d 存在 twist.linear.x / y
